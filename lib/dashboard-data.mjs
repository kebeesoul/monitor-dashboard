...
function totalHistoryFromPoints(points) {
  ...
  return totals.map((point, index) => {
    ...
    const totalDelta = previous ? point.totalViews - previous.totalViews : 0;
    const totalRate = previous && previous.totalViews > 0
      ? Math.round((totalDelta / previous.totalViews) * 10000) / 100
      : 0;
...
export function buildDashboardData(dailyRows, linkRows, generatedAt = new Date().toISOString()) {
  ...
  for (const row of dailyRows.slice(1)) {
    ...
    if (
      ...
      byDay.set(key, { id, date, views, totalViews });
    }
  }
