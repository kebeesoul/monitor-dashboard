...
function growthTrend(history, endDate, days) {
  const requiredPoints = days * 2 + 1;
  const startDate = dateBefore(endDate, days * 2);
  const byDate = new Map(history.map(point => [point.date, point]));
  const points = [];

  for (let index = 0; index < requiredPoints; index++) {
    const point = byDate.get(dateBefore(startDate, -index));
    if (point && Number.isFinite(point.delta)) points.push(point);
  }

  const observedPoints = points.length;
  if (observedPoints !== requiredPoints) {
    return {
      available: false,
  ...
  const previousIncrease = points
    .slice(1, days + 1)
    .reduce((sum, point) => sum + point.delta, 0);
  const currentIncrease = points
    .slice(days + 1)
    .reduce((sum, point) => sum + point.delta, 0);
  ...
  return {
    ...
    currentIncrease,
    previousIncrease,
    changePercent: Math.round(((currentIncrease - previousIncrease) / previousIncrease) * 10000) / 100,
    requiredPoints,
    observedPoints,
    reason: null,
  };
...
function buildMonitoring(history, asOf) {
  ...
  const trend = {
    1: growthTrend(history, asOf, 1),
    7: growthTrend(history, asOf, 7),
    30: growthTrend(history, asOf, 30),
  };
  const alerts = [];
...
export function buildDashboardData(dailyRows, linkRows, generatedAt = new Date().toISOString()) {
  ...
  videos.forEach(video => {
    ...
    if (totalHistory.length) {
      ...
      const currentTotal = video.history.find(point => (
        point.date === dateRange.end && Number.isFinite(point.totalViews)
      ));
      if (currentTotal) video.currentTotalViews = currentTotal.totalViews;
      ...
      video.monitoring.totalTrend = {
        1: growthTrend(totalHistory, dateRange.end, 1),
        7: growthTrend(totalHistory, dateRange.end, 7),
        30: growthTrend(totalHistory, dateRange.end, 30),
      };
    }
  });
...
export function validateDashboardData(data) {
  ...
  const validTrend = (trend, days) => (
    typeof trend?.available === 'boolean'
    && trend.requiredPoints === days * 2 + 1
    && Number.isInteger(trend.observedPoints)
    && trend.observedPoints >= 0
    && trend.observedPoints <= trend.requiredPoints
    && (
      trend.available
        ? (
          Number.isFinite(trend.currentIncrease)
          && Number.isFinite(trend.previousIncrease)
          && trend.previousIncrease > 0
          && Number.isFinite(trend.changePercent)
          && trend.reason === null
        )
        : (
          (trend.currentIncrease === null || Number.isFinite(trend.currentIncrease))
          && (trend.previousIncrease === null || Number.isFinite(trend.previousIncrease))
          && trend.changePercent === null
          && ['insufficient_data', 'non_positive_previous'].includes(trend.reason)
        )
    )
  );
  for (const video of data.videos) {
    if (
      ...
      || ![1, 7, 30].every(days => (
        ...
        && validTrend(video.monitoring?.trend?.[days], days)
      ))
      ...
      || (video.currentTotalViews != null && !Number.isFinite(video.currentTotalViews))
      || (video.monitoring?.totalGrowth != null && ![1, 7, 30].every(days => (
      ...
      || (video.monitoring?.totalTrend != null && ![1, 7, 30].every(days => (
        validTrend(video.monitoring.totalTrend[days], days)
      )))
    ) {
