...
export async function fetchYtmusicCounts(config, fetchImpl = fetch, now = new Date()) {
  ...
  const missing = tracks.filter(track => !Number.isFinite(counts[track.videoId]));
  if (missing.length) {
    throw new Error(`Missing YouTube Music counts: ${missing.map(track => track.videoId).join(',')}`);
  }
  return counts;
}
