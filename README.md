...
- **상단 오른쪽 입력창 (유튜브 URL)**: 유튜브 링크를 그대로 붙여넣고 [조회]를 누르면:
  - 이미 추적 중인 영상이면 → 상세 분석이 뜹니다.
  - 아직 목록에 없는 영상이면 → **[새 곡 등록]** 패널이 열려 시트에 추가할 수 있습니다 (아래 §7 참고).
...
## 7. 새 곡을 URL로 등록하기 — Apps Script 설정 (1회, 약 5분)

상단 입력창에 유튜브 URL을 붙여넣어 **미등록 영상을 구글시트에 바로 추가**하는 기능입니다.
...
**동작:**
- URL 입력 → 미등록 영상이면 [새 곡 등록] 패널이 열림 → (API 키 있으면 아티스트·곡명 자동 채움) → 확인·수정 후 [시트에 등록].
- 데이터는 시트의 **`NewVideos` 탭**(없으면 자동 생성)에 `timestamp / video_id / artist / title / youtube_url` 한 줄로 쌓입니다.
- 같은 video_id는 **중복 추가되지 않습니다**.
- **주의(기대치):** 여기 등록해도 대시보드 목록에는 바로 안 뜹니다. 대시보드는 `DailyDelta`만 읽으므로,
  스크래퍼가 그 영상의 **조회수 행을 `DailyDelta`에 쌓기 시작한 뒤**에 화면에 나타납니다.
  즉 이 등록은 "새 곡을 추적 대상으로 접수"하는 단계입니다.

> 기존 `Links` 탭에 바로 넣고 싶으면 `apps-script.gs` 상단의 `TAB_NAME`을 `'Links'`로 바꾸세요.
> (헤더 이름으로 열을 찾아 쓰므로 Links의 앞 빈 열이 있어도 안전합니다.)

> `SHEET_WRITE_URL`이 비어 있으면 등록 버튼이 "URL 미설정" 안내만 띄우고, 나머지는 정상 동작합니다.
